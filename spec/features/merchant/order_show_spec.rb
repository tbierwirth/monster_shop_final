...
RSpec.describe 'Merchant Order Show Page' do
  describe 'As a Merchant' do
    before :each do
      ...
      @address = @m_user.addresses.create(address: '123 Main St', city: 'Denver', state: 'CO', zip: 80218, nickname: 'Home')
      ...
      @order_1 = @m_user.orders.create!(status: "pending", address_id: @address.id)
      @order_2 = @m_user.orders.create!(status: "pending", address_id: @address.id)
    ...
    it 'I can see order information with only my order items' do
      ...
      expect(page).to have_content(@m_user.addresses.first.address)
      expect(page).to have_content(@m_user.addresses.first.city)
      expect(page).to have_content(@m_user.addresses.first.state)
      expect(page).to have_content(@m_user.addresses.first.zip)
